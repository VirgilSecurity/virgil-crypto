...
def createDockerBuild(slave){
    return {
        node(slave){
            ...
            docker.image("virgilsecurity/virgil-crypto-centos6-env:latest").inside("--user root"){
                // Python
                sh 'source /opt/rh/devtoolset-2/enable && PYTHON_LIBRARIES=/usr/lib64/ PYTHON_INCLUDE_DIR=/usr/include/python2.7 ./utils/build.sh --target=python-2.7'
                ...
                writeFile file: './utils/env.sh', text: ['source /opt/rh/rh-python36/enable', ''].join("\n")
                sh 'source /opt/rh/devtoolset-2/enable && ./utils/build.sh --target=python-3.6'
            }
            organizeFilesUnix('install/python')
            archiveArtifacts('install/**')
        }
