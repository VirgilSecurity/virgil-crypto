    ...
    return {
        node(slave) {
            ...
            withEnv(["PATH=${env.HOME}/.rbenv/bin:${env.PATH}"]){
                writeFile file: './utils/env.sh', text: ['eval "$(rbenv init -)"', 'PYTHON_INCLUDE_DIR=$(python -c "from distutils.sysconfig import get_python_inc; print(get_python_inc())")', 'PYTHON_LIBRARY=$(python -c \'import distutils.sysconfig as sysconfig; print(sysconfig.get_config_var("LIBDIR"))\')'].join("\n")
